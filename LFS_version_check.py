...
Changes in v3.1:
  • Bug fix: the secondary anchor sweep now uses _best() instead of
    "skip if already seen", matching what the per-section fallback already
    did. Without this, a stale section heading (e.g. "Introduction to
    OpenSSH-9.9p2") would shadow a newer download link in the same book
    (e.g. openssh-10.2p1.tar.gz appearing in a sect2 or download list).
...
#
# packaging.version is PEP 440 compliant, which is great for Python packages
# but rejects perfectly normal upstream version strings — most painfully the
# OpenSSH-style "9.9p2" / "10.2p1" form, where 'p' denotes "portable" patch
# level. A bare InvalidVersion exception meant openssh was being silently
# dropped from comparison entirely.
#
# This comparator returns -1/0/+1 (a vs b) using packaging.version when both
# strings are PEP 440 valid, and falls back to a tuple-based comparison
# otherwise. The fallback splits on dots and on the alpha/digit boundary,
# and compares component-wise, with this ordering when types differ at a
# given position:
#       (rc/pre/alpha) < (released)   < (post/p/patch)
# i.e. "9.9rc1" < "9.9" < "9.9p1", so 10.2p1 > 9.9p2 because 10 > 9 in the
# first numeric component and the 'p' suffix doesn't enter into it.
...
_PRE_RANK = {
    ...
    "rc": -1, "pre": -1, "c": -1,
    "":    0,
    "post": 1, "p": 1, "patch": 1, "pl": 1,
}

_VCOMP_SPLIT = re.compile(r"(\d+|[A-Za-z]+)")


def _vtuple(v: str) -> tuple:
    """
    Parse 'X.Y.Zsuffix' into a sortable tuple. Recognised suffixes (case-
    insensitive) get a rank from _PRE_RANK; everything else is treated as a
    plain release (rank 0) and the alphabetic component is appended as-is
    so that two unknown tags still sort lexicographically.
    """
    parts = []
    for chunk in re.split(r"[.\-_+~]", v):
        if not chunk:
            continue
        # split mixed-alphanumeric like '9p2' into ['9', 'p', '2']
        sub = [s for s in _VCOMP_SPLIT.findall(chunk) if s]
        for s in sub:
            if s.isdigit():
                parts.append((1, int(s)))   # numeric components rank "1"
            else:
                tag = s.lower()
                if tag in _PRE_RANK:
                    parts.append((0, _PRE_RANK[tag]))   # known modifier
                else:
                    parts.append((0, 0, tag))           # unknown alpha tag
    return tuple(parts)
...
def _vcmp(a: str, b: str) -> int:
    ...
    Tries packaging.version first (handles all PEP 440 niceties); falls
    back to _vtuple() comparison when either side is not PEP 440 valid
    (e.g. OpenSSH 9.9p2). Never raises.
    """
    try:
        pa = pkg_version.parse(a)
        pb = pkg_version.parse(b)
        if pa < pb: return -1
        if pa > pb: return  1
        return 0
...
def load_html(html_file: str, force_online: bool, refresh: bool) -> str:
    ...
    req = urllib.request.Request(
        BLFS_URL,
        headers={"User-Agent": "LFS-version-checker/3.1 (python urllib)"},
    )
...
def parse_blfs_versions(html: str) -> dict[str, str]:
    ...
    Strategy:
      ...
      4. Secondary sweep over every <a href> in the document to catch
         packages mentioned only in body text — and to UPGRADE entries
         when a newer tarball is linked from the same section (e.g. a
         section heading still says foo-1.2 but the download list points
         at foo-1.3.tar.xz).
    """
    try:
    ...
    # Secondary sweep: every <a href> in the document.
    #
    # Crucially this uses _best() — NOT "skip if already seen".  The book
    # sometimes carries a stale section heading (e.g. "Introduction to
    # OpenSSH-9.9p2") while the download list inside the same section
    # already points at a newer tarball (openssh-10.2p1.tar.gz). The
    # primary pass locks in the heading version; without _best() here,
    # the newer tarball would be silently shadowed.
